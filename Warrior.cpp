...
void Warrior::init(const glm::ivec2 &tileMapPos, ShaderProgram &shaderProgram) {
	heightProp = 1.f / 16.f;
	widhtProp = 1.f / 2.f;
...
void Warrior::setTileMap(TileMap *tileMap)
{
	map = tileMap;
}

void Warrior::setPosition(const glm::vec2 &pos)
{
	posEnemy = pos;
	sprite->setPosition(glm::vec2(float(tileMapDispl.x + posEnemy.x), float(tileMapDispl.y + posEnemy.y)));
}

void Warrior::setTarget(MainPlayer *target){
	player = target;
}
...
void Warrior::update(int deltaTime) {
...
bool Warrior::nextBool(double probability)
{
